...
def make_response(input_dict):
    ...
    first = input_dict.get("first", "explanation")
    if fmt == "special":
...
def parse_response(response, first="explanation", fmt="special"):
    if fmt == "special":
        if first == "explanation":
            explanation, label = response.split("####")
        elif first == "label":
            label, explanation = response.split("####")
        else:
            raise ValueError(f"Invalid order {first}")
        return explanation.strip(), label.strip()
    elif fmt == "natural":
        if first == "explanation":
