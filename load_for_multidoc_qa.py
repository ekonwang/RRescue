...
def parse_args():
    ...
    parser.add_argument("--model_max_length", default=2048, type=int, help="model path")
    # --- data --- #
...
class SupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        with open(args.data_path, "r") as f:
            self.dataset_for_eval = json.load(f)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        ctxs = item["ctxs"]
        label = item["answers"]
        query = utils.make_qa_prompt(ctxs, question)
...
def main():
    ...
    for step, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        temp_list = []
        for input_string, output_string, answers in zip(
